  ...
  export default {
    ...
    data:function () {
      return{
        ...
        member:{},                    //登录变量
        memberForget:{},              //忘记密码变量
        memberRegister:{},            //注册会员变量

        remember:true,                //记住密码
        KEY:"PWD",                    //密码加密传输的盐值
        imageCodeToken:"",            //验证码Token

        //注册框显示的错误信息
        registerMobileValidate:null,            //手机号
        registerMobileCodeValidate: null,       //短信验证码
        registerPasswordValidate: null,         //密码
        registerNameValidate: null,             //昵称
        registerConfirmPasswordValidate: null,  //确认密码
    ...
    //在vue中computed是计算属性，主要作用是把数据存储到内存中，
    // 减少不必要的请求，还可以利用computed给子组件的data赋值。
    computed:{
      //验证手机号修改class
    ...
    methods:{
      ...
      toLoginDiv(){
        ...
        _this.MODAL_STATUS = _this.STATUS_LOGIN;      //模态框登录切换方法
      },

      toRegisterDiv(){
        let _this = this;
        _this.MODAL_STATUS = _this.STATUS_REGISTER;   //模态框注册切换方法
      },

      toForgetDiv(){
        let _this = this;
        _this.MODAL_STATUS = _this.STATUS_FORGET;     //模态框忘记密码切换
      },
      ...
      login () {
        ...
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/web/member/login', _this.member).then((response)=>{
          ...
          if (resp.success) {
            ...
            let loginMember = resp.content;//登录成功的用户信息
            ...
            // 登录成功,调用父组件的setLoginMember，将当前登录成功的loginMember传进去
      ...
      sendSmsForRegister(){
        ...
        let sms = {
          mobile:_this.memberRegister.mobile,//手机号
          use:SMS_USE.REGISTER.key           //用途
        };

        _this.$ajax.get(process.env.VUE_APP_SERVER + '/business/web/member/is-mobile-exits/' + _this.memberRegister.mobile)
          .then(function (res) {
      ...
      sendSmsCode(sms,btnId){
        let _this = this;
        //调用服务端发送短信接口
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/web/sms/send',sms)
          .then(function (res) {
            let response = res.data;
            if(response.success){
              Toast.success("短信已发送");
